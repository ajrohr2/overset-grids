...
function generate_kdtree(grid::CurvilinearGrids.AbstractCurvilinearGrid3D)
    ...
    coords_z = grid.centroid_coordinates.z[grid.iterators.cell.domain]
    pts = Vector{SVector{3, Float64}}(undef, length(coords_x))
    @inbounds for idx in 1:size(pts, 1)
        pts[idx] = SVector(coords_x[idx], coords_y[idx], coords_z[idx])
    end
    return KDTree(pts)
end
